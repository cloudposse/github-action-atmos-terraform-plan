...
class TerraformRegistry {
  ...
  canonicalizeType(type) {
    return type.replace(/^aws_alb$/, "aws_lb").replace(/^aws_alb_/, "aws_lb_");
  }
  ...
  async httpsGetCategories(namespace, name) {
    const provider = await this.fetch("/v2/providers", {
      "filter[namespace]": namespace,
      "filter[name]": name,
    });
    const versions = await this.fetch(provider.data[0].links.self, {
      include: "provider-versions",
    });
    const latest = versions.data.relationships["provider-versions"].data.pop();
    const docs = await this.fetch(`/v2/provider-versions/${latest.id}`, {
      include: "provider-docs",
    });
    for (const doc of docs.included) {
      const {
        attributes: { category, title, subcategory },
      } = doc;
      let prefix = "";
      switch (category) {
        case "data-sources":
...
class NodeMapper {
  ...
  getId(node) {
    const id =
      this.ids[node] ?? (this.ids[node] = (this.count++).toString(this.radix));
    return this.escapeId(`n${id}`);
  }

  escapeId(id) {
    return id.replaceAll("end", "End").replaceAll("class", "Class");
  }
}
...
async function parse(input) {
  ...
  const included = [...included_default, ...(INCLUDE ?? "provider").split(",")];
...
function dumpEdges(edges, stream) {
  const arrange =
    ARROW_DIRECTION == "reverse" ? ([src, dst]) => [dst, src] : (edge) => edge;
